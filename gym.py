...
class Instructor:
    ...
    def can_teach(self, workout_class: WorkoutClass) -> bool:
        ...
        True
        """
        for cer in workout_class.get_required_certificates():
            if cer not in self._certificates:
                return False
        return True
...
class Gym:
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        ins = self._instructors[instr_id]
        work = self._workouts[workout_name]

        if not ins.can_teach(work):
            return False

        if time_point not in self._schedule:
            new = {room_name: (ins, work, [])}
            self._schedule[time_point] = new
            return True
        else:
            for room in self._schedule[time_point]:
                if self._schedule[time_point][room][0].get_id() == ins.get_id():
                    return False
            if room_name not in self._schedule[time_point]:
                self._schedule[time_point][room_name] = (ins, work, [])
                return True
            else:
                return False
    ...
    def offerings_at(self, time_point: datetime) -> List[Tuple[str, str, str]]:
        ...
        for i in self._schedule:
            if i >= time_point:
                for room in self._schedule[i]:
                    tup = self._schedule[i][room]
                    new = (tup[0].name, tup[1].get_name(), room)
                    lst.append(new)
        return lst
    ...
    def instructor_hours(self, time1: datetime, time2: datetime) -> \
        ...
        """
        new = {}
        for iid in self._instructors:
            new[iid] = 0
        for i in self._schedule:
            if time1 <= i <= time2:
                for room in self._schedule[i]:
                    inf = self._schedule[i][room][0].get_id()
                    new[inf] += 1
        return new
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        """
        lst = []
        for idd in self._instructors:
            ins = self._instructors[idd]
            time = self.instructor_hours(time1, time2)[idd]
            salary = time * base_rate + BONUS_RATE * \
                     ins.get_num_certificates() * time
            tup = (idd, ins.name, time, salary)
            lst.append(tup)
        lst.sort()
        return lst
